...
export function FilterBar({
  ...
  return (
    <div className="bg-card/60 backdrop-blur-sm border-b border-card-border px-5 py-2">
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={statusRef}>
          <button
            onClick={() => { setStatusOpen(!statusOpen); setCampaignOpen(false); }}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-all ${
              selectedStatusId
                ? 'bg-accent-soft text-accent border border-accent/20 font-medium'
                : 'bg-muted text-muted-foreground hover:text-foreground border border-transparent'
          ...
          >
            {selectedStatus && (
              <span className="w-2.5 h-2.5 rounded-full ring-1 ring-black/10" style={{ backgroundColor: selectedStatus.color }} />
            )}
            <span>{selectedStatus?.name || 'Status'}</span>
            <svg className={`w-3.5 h-3.5 transition-transform ${statusOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <AnimatePresence>
