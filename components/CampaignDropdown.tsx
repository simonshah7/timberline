...
export function CampaignDropdown({
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            >
                <span className={selectedCampaign ? 'text-foreground truncate' : 'text-muted-foreground'}>
                    {selectedCampaign?.name || 'Select campaign'}
                </span>
                <svg className={`w-3.5 h-3.5 text-muted-foreground transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                        ...
                        <div className="flex-1 overflow-y-auto py-1">
                            ...
                            {searchQuery && !filteredCampaigns.some(c => c.name.toLowerCase() === searchQuery.toLowerCase()) && (
                                <button
                                    ...
                                    className="w-full px-3.5 py-2 text-left text-sm text-accent hover:bg-accent-soft flex items-center gap-2 transition-colors"
                                >
                        ...
                        <div className="p-2 border-t border-card-border">
                            {isAdding ? (
                                ...
                                <button
                                    ...
                                    className="w-full py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted rounded-md border border-dashed border-card-border flex items-center justify-center gap-1.5 transition-colors"
                                >
                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                                    </svg>
